...
const baseStyle = (error: boolean, disabled: boolean, empty?: boolean) => {
	return {
		control: (provided: any, state: any) => {
			...
			let borderColor = `${
				state.isFocused
					? colors.MISC_COLORS.dark
					: colors.MISC_COLORS.background_grey_2
			}!important`;
			if (disabled) {
				borderColor = `${
					colors.BACKGROUND_COLORS.background_disabled
				}!important`;
			} else if (error) {
		...
		multiValue: (provided: any, state: any) => {
			...
			const backgroundColor = state.data.disabled
				? colors.BACKGROUND_COLORS.background_disabled
				: colors.MISC_COLORS.background_grey_1;
		...
		multiValueLabel: (provided: any, state: any) => {
			...
			const color = state.data.disabled
				? colors.TEXT_COLORS.font_disabled
				: colors.TEXT_COLORS.font_blue_grey;
...
export const MentorTypeAheadTheme = {
	baseStyle,
};
