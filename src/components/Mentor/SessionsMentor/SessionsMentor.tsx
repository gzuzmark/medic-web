...
> {
	...
	public render() {
		...
		const navBarText = this.sessionMentor
			? `${
					this.mdp.isDateToday(this.sessionMentor.session.from) ? 'hoy ' : ''
		...
		return (
			<Layout title={'Tutores'}>
				...
				<MentorModalBase
					...
					hideClose={
						this.state.modalCheck.screen === StudentCheckModalScreens.SUCCESS ||
				...
				>
					...
					{!!this.state.modalError ? (
						<ContentModal.Warning
							...
							title={'Lo sentimos, no se pudo agregar al paciente.'}
						/>
					) : null}
				</MentorModalBase>
